...
public:
    ...
    void render(std::unique_ptr<Scene> const& scene, std::unique_ptr<Camera> const& camera);
    ...
    Color3f radiance(std::unique_ptr<Scene> const& scene, Ray const& ray, RandomNumberGenerator& rng, uint32_t current_depth);
};
